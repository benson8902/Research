...
# 定義一個函數來繪製並保存結果
plot_results <- function(results, result_type) {
  lapply(names(results), function(celltype) {
    # 提取結果
    result <- results[[celltype]]
    # 繪製點狀圖
    p = dotplot(result, showCategory = 10) + ggtitle(paste0(celltype, " ", result_type, " enrichment"))
    # 指定保存位置
    filepath <- paste0("C:/Users/benson/Desktop/Research/Result_picture/GSE174188/", result_type, "/", celltype, "_", result_type, "_enrichment.png")
    # 保存圖片
...
# 對 GO 結果進行繪圖
plot_results(go_results, "GO")

# 對 KEGG 結果進行繪圖
plot_results(kegg_results, "KEGG")
